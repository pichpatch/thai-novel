# AGENTS.md — thai-novel project context
...
`.agents/skills/json-transform/SKILL.md` is the project-local JSON authoring skill with two modes:
...
It auto-loads for any Codex session opened in this repo — no install step. To use from a different agent (Cowork, etc.), point that agent at the same `SKILL.md` path.
...
1. **This file (`AGENTS.md`)** — orient on the architecture and decisions
...
5. **`.agents/skills/json-transform/SKILL.md`** — the authoring skill (read before helping with episode JSON)
